...
benchmark against which a surrogate-screened search algorithm can later be
directly checked for optimality rather than merely for exceeding a
literature-reported target. All code, configuration files, and raw
per-call results are released alongside this report for independent
verification.
\end{abstract}
...
for the nuclear fuel case. That surrogate-search comparison, and the
subsequent scale-up to the full $I=80$ configuration space
($4^{80}\approx10^{48}$ configurations, for which brute-force enumeration
is impossible), are left to a follow-up report.
...
The resulting 25-point true Pareto frontier provides a ground-truth
benchmark for the surrogate-screened search comparison planned as the next
step of this research line, and we welcome expert feedback on the
geometry and modeling simplifications identified in Section~9.2 before
proceeding to the full-scale $I=80$ instance.
