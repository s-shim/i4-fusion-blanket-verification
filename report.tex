...
a brute-force-checkable verification instance for surrogate-screened
combinatorial search on PDE-constrained design problems. We describe the
...
framework as a completed nuclear-fuel-assembly case study, recovers a
...
requires a computationally expensive numerical solve. A completed line of
...
problem variants~\cite{chang2026}. This report extends that framework to a
structurally different second engineering domain: discrete fusion
breeding-blanket material layout design, evaluated via Monte Carlo neutron
transport simulation in OpenMC.

Before attempting a full-scale, WCLL-style breeding-blanket design problem
(on the order of $I=80$ discrete modules, for which brute-force enumeration
of the design space is impossible), we first build a small,
brute-force-checkable verification instance: a reduced blanket with $I=4$
toroidal sectors and $K=4$ candidate material recipes per sector, giving
$4^4=256$ total configurations --- small enough to enumerate exhaustively
on a single workstation, and therefore capable of providing a \emph{true}
optimum (or, in the multi-objective case addressed here, a true Pareto
frontier) against which any later approximate search can be checked for
optimality directly, not merely for exceeding an external target.
...
enumeration results, together with a surrogate-screened search verified
directly against that ground truth. It is offered as a self-contained
technical report and as a request for expert feedback on the neutronics
modeling choices before scaling to a full-scale design problem.
...
the DAGMC-enabled OpenMC build must be requested explicitly via the exact
conda build-string match:
\begin{quote}
\ttfamily openmc=0.15.3=dagmc*nompi*
\end{quote}
the default \texttt{openmc} package otherwise resolves to a DAGMC-free,
CSG-only build. Second, the PyPI \texttt{cad\_to\_dagmc} package pulls in
a pip-distributed \texttt{gmsh} wheel with a pre-compiled binary that
requires \texttt{libXft.so.2}, which is absent from a minimal WSL Ubuntu
install and raises:
\begin{quote}
\ttfamily OSError: libXft.so.2: cannot open shared object file
\end{quote}
this was resolved by removing the pip \texttt{gmsh} and installing both
...
and transport engine; (2) a CadQuery-sphere $\to$ \texttt{cad\_to\_dagmc}
$\to$ DAGMC $\to$ OpenMC
...
Following a general combinatorial design formulation, the
...
$\varepsilon\in\{0.15,0.30,0.60,0.90\}$, chosen to span a representative
range from near-natural to highly enriched $^6$Li content.
...
The design intent for the small instance is a torus divided
...
\textbf{Simplification relative to a full-scale design.} This geometry
is a \emph{solid} torus segment, not a hollow, multi-layer (first
wall / breeder zone / structural fraction) shell such as would be needed
for a WCLL-style full-scale ($I=80$) model, and its dimensions ($R=300$~cm,
...
The material cost of a given $^6$Li enrichment $\varepsilon$ follows a
blended-cost model derived from a published MIT S.M.\ thesis material-cost
model \cite{webber2025}:
\[
...
structural fraction, both of which would be part of a more realistic
full-scale design.
\end{enumerate}

\subsection{Relationship to full-scale validation}

This report establishes a true, exhaustively-computed Pareto frontier for
a small instance, intended as a ground-truth benchmark for validating a
surrogate-screened search framework before it is applied to a full-scale
design problem too large to enumerate. The natural next step is to run
the same surrogate-screened Adaptive Randomized Rounding (ARR) search
framework used in a completed nuclear-fuel-assembly case study against
this small instance, and to check its own approximate Pareto frontier
directly against the ground truth reported here --- rather than only
against a literature-reported target, which was the strongest
verification available for the nuclear fuel case. That comparison is
carried out below.
